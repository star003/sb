...
public class widj_term extends PApplet {
...
public void setup() {
		...
		myGis.load();
		tt = PApplet.parseInt(myGis.tekT()*10);
...
public void draw() {
  ...
  if (millis()-m01 > m02) {
    ...
    myGis.load();
	background(255, 255, 204);
	if (PApplet.parseInt(myGis.tekT()*10)> tt ) {
		vect = 1;
		tt = PApplet.parseInt(myGis.tekT()*10);
	}
	else if (PApplet.parseInt(myGis.tekT()*10)< tt ) {
		vect = -1;
		tt = PApplet.parseInt(myGis.tekT()*10);
	}
	else vect = 0;
    ...
    try { 
      String[] list = splitTokens(user[0], "<br>");
		for (byte j=0 ;j<3;j++) {
			...
			tmp[j] = PApplet.parseFloat(list[j*2]);
		}
    }
    catch (Exception e) {
      cur[0] = "n/a";
...
class strelka {
	...
	*/
	...
	strelka (int x1,int y1,int w1) {
		x = x1;
		y = y1;
		h = PApplet.parseInt(w1*0.3f);
		w = w1;
	}
	
	public void st(int r) {
		if (r ==1) fill(0,153,255);
			...
			beginShape();
				...
				vertex(x+h,y);
				vertex(x+h,y+0.7f*w);
				vertex(x+1.5f*h,y+0.7f*w);
					vertex(x+0.5f*h,y+w);
				vertex(x-0.5f*h,y+0.7f*w);
				vertex(x,y+0.7f*w);
				vertex(x,y);
			endShape(CLOSE);
			stroke(0);
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "widj_term" };
    if (passedArgs != null) {
