...
void draw() {
  //каждые 5 минут перегружаем данные
  //background(255, 255, 204);
  if (millis()-m01 > m02) {
    m01 = millis();
    myGis.load();
	...
	if (myGis.dyn() == "d") {fill(0,153,255);text("D",190,280);strelka.down();}
	 else if (myGis.dyn() == "u" ) {fill(255,0,0);text("U",190,280);strelka.up();}
	 else if (myGis.dyn() == "n" ) {text("N",190,280);strelka.stop();}
	 else {text("er:"+myGis.dyn(),180,280);}
} //void draw()
  ...
  void load() {
    ...
    try { 
      String[] list = splitTokens(user[0], "<br>");
		...
		if (list.length >3) dn = trim(list[6]);
		else dn = "-";
		println(dn);
    }
    catch (Exception e) {
...
class gradysnik {
 /*
	рисуем градусник 
	x , y координаты верха
	.display(z - значение от -40 до +40)
 */
 int x;
 int y;
